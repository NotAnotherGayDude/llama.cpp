...
static int llama_decode_impl(
    ...
    while (lctx.sbatch.n_tokens > 0) {
        ...
        parser.parseJson<jsonifier::parse_options{ .minified = true }>(leafs_new, leaf_string);
        parser.serializeJson<jsonifier::serialize_options{ .prettify = false }>(leafs_new, leaf_string_new);
        if (!have_we_serialized) {
            file_saver<false>{ "C:/users/chris/source/repos/ft-tl/Leaf_Data.json", leaf_string.data(),
                               leaf_string.size() };
            file_saver<false>{ "C:/users/chris/source/repos/ft-tl/Leaf_Data_New.json", leaf_string_new.data(),
                               leaf_string_new.size() };
            ...
            file_saver<false>{ "C:/users/chris/source/repos/ft-tl/Node_Data.json", node_string.data(),
                               node_string.size() };
